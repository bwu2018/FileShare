...
def bootstrap_zone(config: DeployConfig) -> None:
    zone = build_zone(origin=config.origin, serial=int(time.time()), ns_ip=config.vps_ip)
    ...
    remote.push_zone_file(local_path, config.ssh_host, config.ssh_user, config.remote_zone_path, config.ssh_port)
